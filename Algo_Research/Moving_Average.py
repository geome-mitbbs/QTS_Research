...
for sector in sp500_by_sector:
    ...
    for k in sp500_by_sector[sector]:
        if k in good_keys:
            print(k)
            algo = back_test_single("portfolio.buy(ticker)",-2500,-1,ticker=k)
...
dict_array_to_csv(list_for_csv,"Bench_Mark.csv",fields=['ticker','return','volatility','draw_down','sharpe'])
...
for sector in sp500_by_sector:
    ...
    for k in sp500_by_sector[sector]:
        if k in good_keys:
            print(k)
            algo = back_test_single(Single_Algos.algos["moving average with support price"],-2500,-1,ticker=k)
            temp_measure = algo.portfolio.get_measures()
            sector_measures={ key:temp_measure[key]+sector_measures.get(key,0) for key in temp_measure}
            all_measures_total={ key:temp_measure[key]+all_measures_total.get(key,0) for key in temp_measure}
            temp_measure['ticker'] = k
...
list_for_csv = [all_measures_total]+all_measures_by_sector+all_measures
dict_array_to_csv(list_for_csv,"Moving_Average_With_Support_Price_Result.csv",fields=['ticker','return','volatility','draw_down','sharpe'])
